...
class PaymentCommonMixin:
    ...
    async def build_topup_success_keyboard(self, user: Any) -> InlineKeyboardMarkup:
        ...
        # Кнопки «Мой баланс» здесь намеренно нет: в боте покупают подписку,
        # а не пополняют счёт, и лишний вход в раздел баланса уводит человека
        # в сторону от покупки. Сам счёт остаётся доступен в Профиле —
        # там его смотрят ради реферальных начислений.
...
async def send_cart_notification_after_topup(
    ...
    *,
    ...
    Возвращает True, если из сохранённой корзины удалось оформить подписку —
    тогда вызывающий провайдер НЕ показывает «Пополнение успешно!»: человек
    покупал тариф, а не пополнял счёт, и про баланс ему знать незачем.
    False означает «подписка не оформлена» — сообщение о зачислении нужно
    показать, иначе после оплаты наступит тишина.

    Само сообщение «Баланс пополнен…» отсюда не шлётся — оно дублировало
    ...
    if cart_data:
        ...
        try:
            # Возвращаем реальный результат: вызывающая сторона по нему решает,
            # показывать ли «Пополнение успешно». Если подписка оформилась,
            # это сообщение лишнее — человек покупал тариф, а не пополнял счёт.
            purchased = await auto_purchase_saved_cart_after_topup(db, user, bot=bot)
            if purchased:
                return True
