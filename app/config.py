...
class Settings(BaseSettings):
    ...
    # Держим окно намерения равным жизни корзины: при прямой оплате тарифа
    # (кнопки способов оплаты сразу на экране тарифа) корзина и намерение
    # создаются одновременно, и более короткое окно давало дыру — оплата
    # позже 30 минут молча падала на баланс, а подписка не оформлялась.
    CART_AUTOPURCHASE_INTENT_TTL_SECONDS: int = 3600
    ...
    # Папка с баннерами разделов: main.png, payment.png, connect.png, referral.png,
    # info.png, support.png. Чего нет — подменяется LOGO_FILE (app/utils/banners.py).
    BANNERS_DIR: str = 'banners'
